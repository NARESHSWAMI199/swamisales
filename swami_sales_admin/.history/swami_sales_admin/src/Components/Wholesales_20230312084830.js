...
const columns: GridColDef[] = [
...

    {
        ...
        renderCell: (params: GridValueGetterParams) => {
            if (params.row.status === 'A') return (<Link to="/" className='link badge badge-success ' style={{textDecoration:'none', color :'white'
        }}><div className='text-center'>Active</div></Link>)
            else return (<Link to='' className='link'>Deactive</Link>)
        }
...
function Wholesale() {
    ...
    return (<>
        ...
        <div className='d-flex justify-content-center text-center' style={{ height: 500, width: '150%' }}>
            <DataGrid
                rows={rows}
                columns={columns}
                pageSize={5}
