...
const columns: GridColDef[] = [
...

    {
        ...
        renderCell: (params: GridValueGetterParams) => {
            if (params.row.status === 'A') return (<Link to="/" key={params.row.id} className='link badge badge-success text-link' ><div className='text-center'>Active</div></Link>)
            else return (<Link to='' key={params.row.id} className='link'>Deactive</Link>)
        }
