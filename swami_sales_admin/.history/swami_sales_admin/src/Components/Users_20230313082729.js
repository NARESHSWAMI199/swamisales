...
const columns: GridColDef[] = [
    ...
    { field: 'is_staff', headerName: 'Staff', width: 90 ,
        renderCell: (params: GridValueGetterParams) => {
            if (params.row.is_staff) return (<div key={params.row.id} className='text-center'><Link to='/' className='badge badge-success link' style={{ textDecoration: 'none', color: 'white' }}><CheckCircleIcon /></Link></div>)
...

    {
       field :'' ,headerName: 'Detail View', width: 100,
        renderCell: (params: GridValueGetterParams) => {
             return (<Link href={`users/${params.row.username}`} className='link text-center' style={{ textDecoration: 'none', color: 'black' }} ><EditIcon/></Link>)
        }
