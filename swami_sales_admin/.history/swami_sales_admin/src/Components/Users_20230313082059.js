...
const columns: GridColDef[] = [
    ...
    { field: 'is_staff', headerName: 'Staff', width: 90 ,
        renderCell: (params: GridValueGetterParams) => {
            if (params.row.is_staff) return (<div key={params.row.id} className='text-center'><Link to='/' className='badge badge-success link' style={{ textDecoration: 'none', color: 'white' }}>Yes</Link></div>)
            else return (<Link to='' className='badge badge-warning link' style={{ textDecoration: 'none', color: 'white' }}>No</Link>)
        }
},
...

    {
       field :'' ,headerName: 'Detail View', width: 100,
        renderCell: (params: GridValueGetterParams) => {
             return (<div key={params.row.id} className='text-center'><Link to={`users/${params.row.username}`} className='btn btn-success link' style={{ textDecoration: 'none', color: 'white' }} >Detail</Link></div>)
        }
